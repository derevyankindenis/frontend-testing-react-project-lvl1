...
describe("negative cases", () => {
  ...
  test("throw exeptions if html already exists", async () => {
    ...
    await expect(pageLoader(FULL_URL, DIR_TO_RUN_TEST)).rejects.toThrow(
      HTMLAlreadyExistsError
    );
  });
  ...
  test("throw exeption if html can't be saved", async () => {
    ...
    await expect(pageLoader(FULL_URL, notExistsDir)).rejects.toThrow(
      NoDirectoryToSaveError
    );
  });
  ...
  test.each([401, 403, 404, 500, 503])(
    ...
    async (code) => {
      ...
      await expect(pageLoader(url, DIR_TO_RUN_TEST)).rejects.toThrow(
        FileCantBeLoadedError
      );
    }
  );
