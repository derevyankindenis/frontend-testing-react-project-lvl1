...
describe('loadAndSave', () => {
  test('load and save file', async () => {
    const scope = nock(URL).get('/').replyWithFile(200, FIXTURE_FILE_PATH, { 'Content-Type': 'text/html' });
  ...
  test("throw error, if file can't be saved", async () => {
    ...
    fs.writeFile = mk;
    nock(URL).get('/').replyWithFile(200, FIXTURE_FILE_PATH, { 'Content-Type': 'text/html' });
    const pathToFile = path.join(DIR_TO_RUN_TEST, FILE_NAME);
    await expect(loadAndSave(URL, pathToFile, (s) => s)).rejects.toThrow(FileCantBeSavedError);
