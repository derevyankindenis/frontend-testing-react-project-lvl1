...
describe("page-loader", () => {
  ...
  describe("positive cases", () => {
    beforeEach(async () => {
      expectedHtml = await fs.readFile(
        getFixturePath("/expected/hexlet-courses-result.html"),
        "utf-8"
      );
      ...
      await Promise.all(
        assetsToCheck.map(async (asset) => {
          ...
          const resultFile = await fs.readFile(
            path.join(tempPath, "ru-hexlet-io-courses_files", asset.out)
          );
  ...
  describe("negative cases", () => {
    ...
    it("should handle filesystem errors", async () => {
      ...
      await expect(
        loadPage("https://ru.hexlet.io/courses", notExistingPath)
      ).rejects.toThrow();
    });

    test.each([401, 403, 404, 500])("should handle %d errors", async (errorCode) => {
      nock("https://ru.hexlet.io").get("/pagewitherror").reply(errorCode);

      await expect(
        loadPage("https://ru.hexlet.io/pagewitherror", tempPath)
      ).rejects.toThrow();
    });
  });
});
